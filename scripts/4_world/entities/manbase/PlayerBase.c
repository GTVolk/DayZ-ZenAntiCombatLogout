modded class PlayerBase
{
	...
	void InformPlayerOfCombatLogout(int willBeKilled)
	{
		...
		GetGame().RPCSingleParam(this, ZEN_ANTI_COMBAT_LOG_RPC, params, true, GetIdentity());
	...
	void SetCombatLogTimer(PlayerBase attacker = NULL, PlayerBase victim = NULL)
	{
		...
		if (attacker != victim && victim != NULL && attacker != NULL && victim.GetIdentity() != NULL && attacker.GetIdentity() != NULL)
		{
			...
			if (!victim.m_ShotAtUsFirst.Contains(attacker.GetIdentity().GetPlainId())) 
			{
				attacker.m_ShotAtUsFirst.Set(victim.GetIdentity().GetPlainId(), false);
				victim.m_ShotAtUsFirst.Set(attacker.GetIdentity().GetPlainId(), true);
			}
		}
	...
	{
		if (enemy.GetIdentity() == NULL || GetIdentity() == NULL)
			return false;

		bool weShotFirst = false;
		enemy.m_ShotAtUsFirst.Find(GetIdentity().GetPlainId(), weShotFirst);
